...
class OCRConfig:
    ...
    def from_config_file(cls, config_path: str = "config.ini") -> "OCRConfig":
        ...
        if os.path.exists(config_path):
            ...
            return cls(
                tesseract_cmd=config.get(
                    "OCR", "tesseract_cmd", fallback=default_tesseract
                ),
                language=config.get("OCR", "language", fallback="jpn"),
                engine=config.get(
                    "OCR", "engine", fallback=OCREngineType.EASYOCR.value
                ),
                enable_logging=config.getboolean(
                    "OCR", "enable_logging", fallback=True
                ),
            )
        logging.info(
            "config.ini not found. Using defaults: language=jpn, engine=easyocr"
        )
        return cls(
...
class OCREngine:
    ...
    def recognize(self, image: Image.Image, mode: str, psm_mode: str) -> OCRResult:
        """Run OCR on a preprocessed image and return the recognition result."""
...
class TesseractEngine(OCREngine):
    ...
    def __init__(self, config: OCRConfig):
        """Initialize the Tesseract engine with runtime configuration."""
        self.config = config
        pytesseract.pytesseract.tesseract_cmd = config.tesseract_cmd

    def recognize(self, image: Image.Image, mode: str, psm_mode: str) -> OCRResult:
        """Recognize text by using Tesseract with the selected PSM mode."""
        custom_config = f"--psm {psm_mode} --oem 1"
        if hasattr(sys, "frozen"):
            custom_config += " --encoding UTF8"

        raw_text = pytesseract.image_to_string(
            image, lang=self.config.language, config=custom_config
        )
        ...
        if hasattr(sys, "frozen"):
            try:
                ...
                raw_text = raw_text.encode(system_encoding, errors="ignore").decode(
                    "utf-8"
                )
...
class EasyOCREngine(OCREngine):
    ...
    def __init__(self):
        """Prepare the EasyOCR reader and availability status."""
        self.reader = None
        self.available = False
        self._init_reader()

    def _init_reader(self) -> None:
        """Initialize EasyOCR reader if the dependency is available."""
        if importlib.util.find_spec("easyocr") is None:
            logging.warning(
                "EasyOCR is not installed. Fallback to Tesseract will be used."
            )
            return
    ...
    def recognize(self, image: Image.Image, mode: str, psm_mode: str) -> OCRResult:
        """Recognize text and aggregate confidence from EasyOCR detections."""
        ...
        confidences = [
            ...
            if len(entry) > 2 and isinstance(entry[2], (float, int))
        ]
...
class ImageProcessor:
    ...
    def __init__(self, config: OCRConfig):
        """Build OCR engines used by image preprocessing and extraction pipeline."""
        self.config = config
    ...
    def _clean_japanese_text(self, text: str) -> str:
        """Remove control characters and keep Japanese/basic-latin characters."""
        text = re.sub(r"[\x00-\x1F\x7F-\x9F]", "", text)
        valid_chars = r"[　-〿]" r"|[぀-ゟ]" r"|[゠-ヿ]" r"|[一-鿿]" r"|[＀-￯]" r"|[ -~]"
        return "".join(char for char in text if re.match(valid_chars, char))

    def _select_engine(self) -> OCREngineType:
        """Resolve configured engine safely with EasyOCR as default fallback."""
        try:
    ...
    def _next_preprocess_mode(self, preprocess_mode: str) -> str:
        """Return the next preprocess mode used for retry attempts."""
        fallback_order = {
    ...
    def extract_text(
        self, image: Image.Image, mode: str, psm_mode: str, preprocess_mode: str
    ) -> OCRResult:
        ...
        try:
            # 1) Run OCR with the requested primary engine. / 1) 指定されたプライマリエンジンでOCRを実行
            if requested_engine == OCREngineType.TESSERACT:
                result = self.tesseract_engine.recognize(
                    processed_image, mode, psm_mode
                )
            else:
                result = self.easyocr_engine.recognize(processed_image, mode, psm_mode)

                # 2) Retry EasyOCR once with an alternative preprocessing mode when quality is low.
                # 2) 品質が低い場合、別の前処理モードでEasyOCRを1回再試行
                if self._needs_easyocr_retry(result):
                    ...
                    retry_result = self.easyocr_engine.recognize(
                        retry_image, mode, psm_mode
                    )
            ...
            # 3) Fall back to Tesseract when primary OCR path fails. / 3) プライマリ失敗時はTesseractにフォールバック
            logging.warning(
                f"Primary OCR engine failed ({requested_engine.value}): {str(e)}"
            )
    ...
    def extract_numbers_and_calculate(text: str) -> Tuple[Optional[str], Optional[int]]:
        ...
        if not numbers:
            return (
                None,
                None,
            )  # Return None if no numbers are found    / 数値が見つからない場合はNoneを返す
        ...
        try:
            ...
            for line in lines:
                if line.strip():
                    ...
                    cleaned_line = "".join(
                        char for char in cleaned_line if re.match(valid_chars, char)
                    )
                    if cleaned_line:
...
class OCRWindow:
    ...
    def __init__(self, root: tk.Tk):
        """Initialize UI state, OCR processors, and clipboard monitoring."""
    ...
    def _log_effective_settings(self) -> None:
        ...
        if "jpn" in self.config.language:
            try:
                ...
                if "jpn" not in langs:
                    logging.warning(
                        "Tesseract Japanese language pack (jpn) is missing."
                    )
            except Exception as e:
    ...
    def setup_window(self, root: tk.Tk) -> None:
        ...
        self.root.title(
            "クリップボード スカウター"
        )  # Set the title of the application / アプリケーションのタイトルを設定
        self.root.geometry(
            "420x420"
        )  # Set the window size              / ウィンドウサイズを設定
        self.root.resizable(
            False, False
        )  # Disable resizing                 / サイズ変更を無効化

        # Variables / 変数
        self.mode = tk.StringVar(
            value=ProcessingMode.CALCULATION.value
        )  # Current processing mode / 現在の処理モード
        self.psm_mode = tk.StringVar(
            value=PSMMode.SPARSE_TEXT.value
        )  # Current PSM mode        / 現在のPSMモード
        self.preprocess_mode = tk.StringVar(
            value=f"{PreprocessMode.SCREEN_DEFAULT.value}: {PreprocessMode.SCREEN_DEFAULT.display_text}"
        )
        self.dark_mode = tk.BooleanVar(
            value=False
        )  # Dark mode toggle        / ダークモードの切り替え
    ...
    def create_mode_controls(self) -> None:
        ...
        self.mode_toggle.config(
            width=10
        )  # Fixed width for consistent UI / 一貫したUIのための固定幅
        ...
        self.psm_menu.config(
            width=12
        )  # Slightly wider for longer labels / 長いラベルのため少し幅広に設定
    ...
    def configure_dropdown_menu(self, menu: tk.Menu) -> None:
        ...
        theme = (
            Theme.DARK if self.dark_mode.get() else Theme.LIGHT
        )  # Select theme based on current mode / 現在のモードに基づいてテーマを選択
        ...
        menu.configure(
            ...
            activebackground=theme[
                "active_bg"
            ],  # Active item background / アクティブ項目の背景
            activeforeground=theme[
                "active_fg"
            ],  # Active item text color / アクティブ項目の文字色
    ...
    def configure_option_menu_style(self, option_menu: ttk.OptionMenu) -> None:
        ...
        theme = (
            Theme.DARK if self.dark_mode.get() else Theme.LIGHT
        )  # Use theme-specific colors / テーマごとの色を使用
    ...
    def set_theme(self) -> None:
        ...
        theme = (
            Theme.DARK if self.dark_mode.get() else Theme.LIGHT
        )  # Select theme based on dark mode state / ダークモードの状態に基づいてテーマを選択

        # Configure window and frame backgrounds / ウィンドウとフレームの背景色を設定
        self.root.configure(
            bg=theme["window_bg"]
        )  # Set main window background / メインウィンドウの背景を設定
        self.button_frame.configure(
            bg=theme["frame_bg"]
        )  # Set button frame background / ボタンフレームの背景を設定
        self.label_result.configure(
            bg=theme["window_bg"], fg=theme["window_fg"]
        )  # Update label colors / ラベルの色を更新
        ...
        self.result_display.configure(
            ...
            insertbackground=theme[
                "text_fg"
            ],  # Cursor color in text area / テキストエリア内のカーソル色
        )
        ...
        self.style.configure(
            ...
            padding=(
                10,
                5,
            ),  # Padding for larger click area / より大きなクリックエリアのためのパディング
        )
        ...
        self.style.configure(
            ...
            background=theme[
                "option_bg"
            ],  # Dropdown background color     / ドロップダウンの背景色
            foreground=theme[
                "option_fg"
            ],  # Dropdown text color           / ドロップダウンの文字色
            relief="flat",  # Flat style                    / フラットスタイル
            padding=(
                5,
                2,
            ),  # Padding for better click area / より良いクリックエリアのためのパディング
            arrowcolor=theme[
                "option_fg"
            ],  # Arrow icon color              / 矢印アイコンの色
        )
        self.mode_toggle.configure(
            style="Mode.TMenubutton"
        )  # Apply style to mode toggle / モード切り替えにスタイルを適用

        # Define interactions for dropdown menus / ドロップダウンメニューの動作を定義
        self.style.map(
            "Mode.TMenubutton",
            background=[
                ("active", theme["active_bg"]),  # Active background  / アクティブ背景
                ("pressed", theme["active_bg"]),
            ],  # Pressed background / 押下時の背景
            foreground=[
                ("active", theme["active_fg"]),  # Active text color  / アクティブ文字色
                ("pressed", theme["active_fg"]),
            ],  # Pressed text color / 押下時の文字色
        )
        ...
        self.style.map(
            "TOptionMenu",
            background=[
                ("active", theme["active_bg"]),  # アクティブな背景色
                ("pressed", theme["active_bg"]),
            ],  # 押下中の背景色
            foreground=[
                ("active", theme["active_fg"]),  # アクティブな文字色
                ("pressed", theme["active_fg"]),
            ],  # 押下中の文字色
        )
        ...
        self.configure_option_menu_style(
            self.mode_toggle
        )  # モード切り替えメニューのスタイル設定
    ...
    def update_mode(self, _: str) -> None:
        ...
        logging.debug(
            f"Mode changed to: {self.mode.get()}, Display text: {current_mode_text}"
        )
